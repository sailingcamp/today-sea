...
`.trim();
  ...
  if (windChange >= 4) {
    parts.push("この先8時間で風速の変化が大きくなる可能性があります。出艇前だけでなく、帰着時の風も意識してください。");
  } else if (windChange >= 2) {
    parts.push("この先8時間で風が少し変化しそうです。活動中も風の上がり方を確認しましょう。");
  } else {
    parts.push("この先8時間の大きな風速変化は少なめです。ただし、風向の変化もあわせて確認しましょう。");
  }
...
function showScoreInfo() {
  showModal(
    ...
    `
    <p>
    海スコアは、海のコンディションを分かりやすく把握するための参考指標です。
    </p>
